...
enum in { up, down, left, right, center, song_end } inputs;
...
static void gpio_task_example(void* arg) {
  ...
  for (;;) {
    if (xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
      // depending on what state is and what button was pressed, may need to
      // change state
      switch (state) {
        case menu:
          if (io_num == up) {
...
enum state_machine { menu, playing } state;
...
void app_main() {
  ...
  io_conf.intr_type =
      GPIO_PIN_INTR_POSEDGE;  // should this be GPIO_PIN_POSEDGE???

  // create a queue to handle gpio event from isr
  gpio_evt_queue = xQueueCreate(
      10, sizeof(uint32_t));  // creates a queue of 10 uint32_t variables
  // start gpio task
